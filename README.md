...
| **小旭恐惧指数(XXFI)** | GitHub 跨仓库（CORS 代理优先三级加速，见下） | **实时拉取，不缓存**（每次刷新取最新，卡片显示数据日期） |
...
三个跨仓数据源（xiaoxu-fear / cmb-tracker）采用**三级切换，用户 CORS 代理优先**：

| 优先级 | 路径 | 特点 |
|---|---|---|
| ① 用户代理 | 设置页「CORS 代理 URL」填写的地址（自建 Cloudflare Worker） | **无缓存、实时** |
| ② 直连 | raw.githubusercontent.com | 无缓存，国内可能超时 |
| ③ jsdelivr | `cdn.jsdelivr.net/gh/仓库@main/路径` | 快但有 **12h 服务端缓存**，仅最后兜底（宁旧勿无） |

> ⚠️ jsdelivr 的 12h 缓存会导致 XXFI/秋哥/银行五维显示旧数据，故仅作极端兜底；**高频实时数据必须走用户代理或直连**。
