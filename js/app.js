...
const App = (function() {
    ...
    function reinvestDividend(dividendId) {
        ...
        StorageManager.updateDividendRecord(dividendId, {
            status: 'reinvested',
            reinvest: { shares: reinvestShares, price: reinvestPrice, date: getTodayStr() }
        });
    ...
    function getTodayStr() {
        // 本地时区日期（toISOString 按 UTC，北京时间凌晨会取到昨天）
        const d = new Date();
    ...
    async function cloudBackup() {
        ...
        try {
            ...
            StorageManager.saveConfig(config);
            UIManager.showToast(provider === 'gitee' ? `备份成功：${locator}` : '备份成功！', 'success');
        } catch (e) {
    ...
    async function cloudImport() {
        ...
        try {
            ...
            let data;
            if (provider === 'gitee') {
                // Gitee：时间戳快照策略 → 弹单选列表（默认勾选最新一份，可切换更早版本，无需合并）
                const files = await APIManager.giteeListSnapshots(cfg);
                if (!files.length) throw new Error('Gitee 仓库中暂无备份快照，请先备份一次（或检查仓库/路径是否正确）');
                const name = await pickGiteeSnapshot(files);
                if (!name) { btn.disabled = false; btn.textContent = '📥 从云端导入'; return; }
                data = await APIManager.giteeFetchSnapshot(cfg, name);
            } else {
                data = await APIManager.cloudFetch(cfg);
            }
    ...
    // Gitee 快照单选列表（默认勾选最新一份，可切换更早版本；无多选/合并）
    // 返回选中文件名；用户取消返回 null
    function pickGiteeSnapshot(files) {
            ...
            overlay.innerHTML = `
                <div class="modal-content" style="max-width:540px">
                    <div class="modal-header">
                        ...
                        <button class="modal-close" id="snap-close">&times;</button>
                    </div>
                    <div class="modal-body">
                        <p class="form-hint" style="margin-bottom:10px">云端共 <b>${files.length}</b> 份快照（最新在上），默认勾选最新一份，可切换更早版本。</p>
                        <div style="max-height:52vh;overflow:auto;margin:10px 0">
                            ${files.map((f, i) => `
                                <label style="display:flex;align-items:center;gap:8px;padding:8px 10px;border:1px solid #e5e7eb;border-radius:6px;margin-bottom:6px;cursor:pointer;background:#fff">
                                    <input type="radio" name="snap" value="${esc(f.name)}" ${i === 0 ? 'checked' : ''}>
                                    <span style="font-weight:600;font-size:13px">${esc(f.name)}</span>
                                    <span style="color:#9ca3af;font-size:12px;margin-left:auto">${f.size ? Math.max(1, Math.round(f.size / 1024)) + ' KB' : ''}</span>
                                </label>`).join('')}
                        </div>
                        <div style="display:flex;gap:10px;justify-content:flex-end">
                            <button class="btn btn-success" id="snap-ok">📥 拉取所选快照</button>
                            <button class="btn" id="snap-cancel">取消</button>
                        </div>
                    </div>
                </div>`;
            document.body.appendChild(overlay);
            const done = (v) => { if (overlay.parentNode) overlay.parentNode.removeChild(overlay); resolve(v); };
            overlay.querySelector('#snap-close').onclick = () => done(null);
            overlay.querySelector('#snap-cancel').onclick = () => done(null);
            overlay.querySelector('#snap-ok').onclick = () => {
                const sel = overlay.querySelector('input[name="snap"]:checked');
