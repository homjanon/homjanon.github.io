...
const APIManager = (function() {
    ...
    async function getEastMoneyFundNav(code) {
        ...
        const navDate = last.x ? fmtDate(last.x) : '';
    ...
    // 本地时区日期 YYYY-MM-DD（东财 Data_netWorthTrend 的 x 是北京时间零点时间戳，
    // 用 toISOString()(UTC) 切片会晚一天，必须按本地时区格式化）
    function fmtDate(ts) {
    ...
    // ---- Gitee（私人仓，时间戳新文件策略，对齐 jingjishi 9.4） ----
    // 每次写入 user-data-YYYYMMDD-HHMMSS.json 新文件（POST 新建、永不需要 sha，天然免疫 400/409）；
    // cloudPath 语义 = 目录（旧值 data/user-data.json 自动截取目录，零迁移）；旧文件保留当历史备份。
    ...
    function giteeDir(config) {
        let dir = (config.cloudPath || 'data/user-data.json').trim();
        if (/\.json$/i.test(dir)) {          // 兼容旧配置：填的是文件路径，取其所在目录
            const i = dir.lastIndexOf('/');
            dir = i >= 0 ? dir.slice(0, i + 1) : '';
        }
        return dir && !/\/$/.test(dir) ? dir + '/' : dir;
    }
    function giteeBaseUrl(config, name) {
        const { cloudRepo: repo } = config;
        return `https://gitee.com/api/v5/repos/${repo}/contents/${giteeDir(config)}${name || ''}`;
    }
    // Gitee GET 带鉴权双保险：?access_token= 查询串 → 401/403 降级 Bearer 头；no-store 防缓存
    async function giteeGet(config, name) {
        const token = config.cloudToken;
        const url = giteeBaseUrl(config, name);
    ...
    async function giteeBackup(data, config) {
        const { cloudRepo: repo, cloudToken: token } = config;
        if (!token || !repo) throw new Error('请填写 Gitee 私人令牌与仓库(owner/repo)');
        const content = utf8ToBase64(JSON.stringify(data, null, 2));
        const postCreate = (n) => fetch(giteeBaseUrl(config, n), {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({ content, message: 'investment-tracker backup', access_token: token })
        });
        // 时间戳文件名 → POST 新建（无需 sha，天然免疫 400/409）
        let name = giteeStampName();
        let resp = await postCreate(name);
        if (resp.ok) return name;
        const status = resp.status;
        const errText = await resp.text().catch(() => '');
        // 同秒撞名（400 文件名已存在 / 409 / 422）→ 追加随机数重试一次
        if (status === 400 || status === 409 || status === 422) {
            name = name.replace('.json', '-' + Math.floor(Math.random() * 900 + 100) + '.json');
            const r2 = await postCreate(name);
            if (r2.ok) return name;
            const t2 = await r2.text().catch(() => '');
            throw new Error(`Gitee ${r2.status}: ${t2.substring(0, 200)}`);
        }
        if (status === 401 || status === 403) throw new Error('Gitee 鉴权失败：令牌无效或权限不足（请确认勾选 projects 权限）');
        throw new Error(`Gitee ${status}: ${errText.substring(0, 200)}`);
    }
    // 列目录 → 全部 user-data-*.json 快照，按时间倒序（最新在前）
    async function giteeListSnapshots(config) {
        const { cloudRepo: repo, cloudToken: token } = config;
        if (!token || !repo) throw new Error('请填写 Gitee 私人令牌与仓库(owner/repo)');
        const resp = await giteeGet(config, '');
        if (resp.status === 404) return [];
        if (resp.status === 401 || resp.status === 403) throw new Error('Gitee 鉴权失败：令牌无效或权限不足（请确认勾选 projects 权限）');
        if (!resp.ok) { const err = await resp.text().catch(() => ''); throw new Error(`Gitee ${resp.status}: ${err.substring(0, 200)}`); }
        const list = await resp.json().catch(() => []);
        if (!Array.isArray(list)) return [];
        let files = list.filter(f => f && f.name && /^user-data-\d{8}-\d{6}(-\d+)?\.json$/.test(f.name))
                        .sort((a, b) => b.name.localeCompare(a.name));
        // 兜底：无时间戳快照但存在旧固定文件 user-data.json（老格式兼容）
        if (!files.length && list.some(f => f && f.name === 'user-data.json')) {
            files = list.filter(f => f && f.name === 'user-data.json');
        }
        return files.map(f => ({ name: f.name, size: f.size || 0 }));
    }
    // 拉取指定快照内容
    async function giteeFetchSnapshot(config, name) {
        const resp = await giteeGet(config, name);
        if (!resp.ok) { const err = await resp.text().catch(() => ''); throw new Error(`Gitee ${resp.status}: ${err.substring(0, 200)}`); }
        const j = await resp.json().catch(() => ({}));
        if (!j || !j.content) throw new Error('Gitee 读取成功但未返回文件内容，请检查仓库/路径是否正确');
        return JSON.parse(base64ToUtf8(j.content || ''));
    }
    // 拉取最新一份（列目录取时间戳最新；供自动同步等调用）
    async function giteeFetch(config) {
        const files = await giteeListSnapshots(config);
        if (!files.length) throw new Error('Gitee 仓库中暂无备份快照，请先在「同步方式」下备份一次（或检查仓库/路径是否正确）');
        return giteeFetchSnapshot(config, files[0].name);
    }
    ...
    return {
        ...
        giteeListSnapshots, giteeFetchSnapshot,
